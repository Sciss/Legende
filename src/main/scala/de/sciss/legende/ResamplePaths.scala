...
import de.sciss.legende.Prepare.{readDijkstra, writeDijkstra, shouldWrite}
...
object ResamplePaths {
  ...
  def mkSegMod(fSoundIn: File, fSoundInA: File, fSoundInB: File, outGain: Double, fRouteA: File, fRouteB: File, fSegModRvs: File)
              (implicit config: Config): Future[Unit] = {
    var routeA0     = readDijkstra(fRoute = fRouteA)
    var routeB0     = readDijkstra(fRoute = fRouteB)
//    println(s"Route.length = ${route0.length}")
//    if (route0.length < 100) println(route0.mkString("Vector(", ", ", ")"))
    val specIn      = AudioFile.readSpec(fSoundIn)
    val specInA     = AudioFile.readSpec(fSoundInA)
    val specInB     = AudioFile.readSpec(fSoundInB)
    val overlapFrames = (specInB.numFrames + specInA.numFrames - specIn.numFrames).toInt
    val overlapH    = overlapFrames/2
...
//      println(periods.take(50))
      val freqN0    = ValueDoubleSeq(periods.map(1.0 / _): _*)
